...
/*
 * The following functions are taken from the CDM C library.
 */
...
{
    return std::tolower((CDM_UniChar) ch, std::locale("en_US.utf8"));
}
    ...
    int nocase)			/* 0 for case sensitive, 1 for insensitive */
{
	...
	if (p == '*') {
		...
		if ((p != '[') && (p != '?') && (p != '\\')) {
			...
			while (*str) {
			    ...
			    if (ch2==ch1 || ch2==CDM_UniCharToLower(ch1)) {
				break;
			    }
			    str += charLen;
			}
	...
	if (p == '[') {
		...
		if (UCHAR(*pattern) < 0x80) {
		    ...
		    if (nocase) {
			startChar = CDM_UniCharToLower(startChar);
		    }
		}
		if (*pattern == '-') {
		    pattern++;
			...
			if (nocase) {
			    endChar = CDM_UniCharToLower(endChar);
			}
		    }
	...
	if (nocase) {
	    if (CDM_UniCharToLower(ch1) != CDM_UniCharToLower(ch2)) {
		return 0;
	    }
	} else if (ch1 != ch2) {
	    return 0;
